...
class Habits(models.Model):
    ...
    good_habit = models.BooleanField(
        default=False,
        verbose_name='признак приятной привыки'
    )
    habit_link = models.ForeignKey(
        'self',
        on_delete=models.SET_NULL,
        verbose_name='связвнная привычка',
        null=True,
        blank=True
    )
    periodicity = models.SmallIntegerField(
        verbose_name='периодичность - (количество повторений в неделю)'
    )
    ...
    public = models.BooleanField(
        default=False,
        verbose_name='признак публичность'
    )
